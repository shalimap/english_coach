...
import 'package:englishcoach/pages/trial/screens/video_screen_web.dart';
...
class TrialModuleDetailScreen extends StatefulWidget {
  static const routeName = '/trailmodule-detail';

  @override
  _TrialModuleDetailScreenState createState() =>
      _TrialModuleDetailScreenState();
}

class _TrialModuleDetailScreenState extends State<TrialModuleDetailScreen>
    with TickerProviderStateMixin<TrialModuleDetailScreen> {
  var _isInit = true;
  var topicTest = true;
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<TrialExamples>(context).fetchExamples(moduleId);
      Provider.of<TrialStructures>(context).fetchStructures(moduleId);
      Provider.of<TrialVideo>(context, listen: false)
          .fetchVideos(moduleId)
          .then((_) {
        final data =
            Provider.of<TrialVideo>(context, listen: false).findVideo(moduleId);
        videoUrl = data.vidUrl;
      });
  ...
  Widget build(BuildContext context) {
    ...
    final loadedModule = moduleData['loadedModule'];
    final loadedExamples =
        Provider.of<TrialExamples>(context).findExamples(moduleId);
    final loadedStructures =
        Provider.of<TrialStructures>(context).findStructures(moduleId);

    // final unlockCount =
    //     Provider.of<TrialModuleUnlock>(context, listen: false).countLength;
    // final latestUnlockedMod =
    //           Provider.of<TrialModules>(context, listen: false)
    //               .nextModuleId((unlockCount - 1));
    // final nextUnlockMod = Provider.of<TrialModules>(context, listen: false)
    //     .nextModuleId(unlockCount);

    //print(_isUnlocked.toString());
      ...
      child: Scaffold(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  ...
                  if (moduleId != latestUnlockMod) {
                    ...
                    //!!error hereee check nextUnlockMod=null

                    if (nextUnlockMod != null) {
                      var _isUnlocked =
                          Provider.of<TrialModuleUnlock>(context, listen: false)
        ...
        floatingActionButton: ScaleTransition(
          ...
          child: FloatingActionButton.extended(
              ...
              tooltip: "Tests",
              onPressed: () {
                Navigator.pushNamed(context, TrialReportscreen.routename,
                    arguments: {
                      'userId': userId,
                      'mod': moduleId,
                      'modDesc': loadedModule.modDescription,
                      'modSpcl': loadedModule.modSpecialnote,
                      'modName': loadedModule.modName,
                      'countUnlock': unlockCount,
